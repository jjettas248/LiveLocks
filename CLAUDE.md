...
npx tsx server/mlb/pregame/composition/moundPlateTargets.test.ts # Cross-Radar (neutral composition layer, not inside either engine): pure Mound→Plate HR-target join/rank/dedupe + isMoundCompositionEligible pregame-only lifecycle gate (locked/graded/live/final/postponed Mound cards get zero suggestions) — gameId+pitcherId join, whitespace-normalized dedupe, deterministic HR-score-first ranking with overall-score/slot/batterId tie-breaks, defensive track-tier exclusion, capped at 3, never mutates inputs
npx tsx server/mlb/pregame/composition/enrichMoundResponse.test.ts # Cross-Radar response enrichment: isPlateCompositionEligible layers a pregame-only lifecycle check ON TOP of Plate's REAL canonical isPublicPregameSignal gate (proves a locked/graded/live/final/suspended Plate candidate that isPublicPregameSignal's retention branch would still call "public" is nonetheless excluded), wraps an already-built MoundRadarResponse without mutating it, canonical fields (score10/tier/moundDirection/ordering/diagnostics) pass through byte-identical
npx tsx server/mlb/pregame/composition/loadPregameCompositionContext.test.ts # Cross-Radar Plate snapshot loader: missing/date_mismatch/available(+buildId)/load_error state classification via an injectable snapshot accessor, MOUND_PLATE_TARGET_SUGGESTIONS_ENABLED flag reflects the env var, never throws
npx tsx server/mlb/pregame/composition/composeMoundResponse.test.ts # Cross-Radar route orchestration: disabled flag reports state="disabled" (not "missing"), a Plate loader failure degrades to empty suggestions without altering canonical Mound fields, a genuine buildMoundResponse exception propagates rather than being mislabeled a Plate failure, buildMoundResponse is called exactly once and structurally before the try block, exactly ONE console.log call carrying [MOUND_PLATE_COMPOSITION] and ZERO console.warn/console.error calls on every path incl. failure, failureKind/failureMessage carried in that single record with a 200-char cap and no raw Error/stack ever logged
npx tsx server/mlb/pregame/composition/moundRouteWiring.test.ts # Cross-Radar structural proof: all 3 real Mound routes (mound-power-radar, all-starters, admin/debug) assign `resp` from composeMoundResponseWithPlateTargets and never call buildMoundResponse directly (no uncomposed response construction)
npx tsx client/src/lib/mlb/moundPlateTargetsDisplay.test.ts # Cross-Radar client display logic: all-HR vs mixed vs all-fallback heading selection (a single overall-fallback entry must never earn the HR-specific heading), safe hrScore formatting with no non-null assertion, graceful fallback when rankingBasis claims "home_runs" but hrScore is null/NaN/Infinity
...
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/plateHrV2FeaturesV3.test.ts # PR7A.2 plate_hr_v2_features_v3 contract (shadow/additive): V3 = V2 + pitcherDiscipline (vsL/vsR history + pitcherPitches; no prediction-specific vsHand/batterHand), extended contactOpportunity (chase/zoneContact pinned literal null; count leaves non-neg-int), zoneLocation explicit-unavailable record, V3 dataQuality retrosheetDiscipline block with typed null reasons; CURRENT stays V2; strict; authorized projection strips market+zoneLocation; parseAuthorizedProjection routes V3
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/plateDisciplineNoLocationFlag.test.ts # PR7A.2 PLATE_DISCIPLINE_NO_LOCATION_V1_ENABLED fail-closed parser + composite capture gate (requires BOTH the discipline flag AND the master forward-capture flag)
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/retrosheetDisciplineEvidence.test.ts # PR7A.2 retrosheet_discipline evidence-kind validator: single-actor (batter XOR pitcher) payload, closed keys, non-neg int counts, batter+split consistency (swings=whiffs+contacts overall AND per-hand, split totals≤overall), pitcher pitches denominator (calledStrikes≤pitches NOT ≤BF; positive calledStrikes>BF case), immutable pitcher vsL/vsR history (no vsHand/batterHand), hardened provenance (unique gameIds, window, EXACT attribution notice, strict ISO, below_sequence_coverage↔sequenceFloorMet, null-reasons vs overallQuality=full), retrosheet-only provider
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/retrosheetDisciplineIsolation.test.ts # PR7A.2 structural isolation + location-blindness: pure evidence module has zero imports + no Savant/MLB-Stats reference; new v3 leaf names carry no forbidden zone terms; authorized V3 projection strips zoneLocation/market; unavailable zoneLocation is all-null
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/retrosheetDisciplineReadIntegrity.test.ts # PR7A.2 retrosheet_discipline strict training-read integrity: rejects unauthorized provider, non-verified_as_of, entity/actor mismatch, wrong batter id, schema/version mismatch, retrosheet evidence on a V1/V2 prediction; V3 is FAIL-CLOSED training-ineligible (v3_discipline_binding_not_implemented) even when structurally valid; V3 recentContactForm without valid contact_events stays unreadable
...
npx tsx server/services/nbaStatsService.season.test.ts # Pregame Targets PR5 — provider season support: omitted season → current (byte-equivalent request preserved), explicit prior season used verbatim (reaches provider)
npx tsx server/pregameTargets/ingestion/rawSnapshotIdentity.test.ts # Pregame Targets PR5 — raw-snapshot identity: deterministic content hash + snapshotId, semantic-key component separation; audit-4 CAPTURE identity — same payload at different observation instants → same contentHash but DIFFERENT capture ids (A→B→A recurrence), contentHash stays a pure payload hash (uncontaminated)
npx tsx server/pregameTargets/ingestion/nbaGameLogAdapter.test.ts # Pregame Targets PR5 — adapter: incomplete/empty/malformed classified (never fabricated), observed vs missing (null stat) vs observed_zero distinct, SEMANTIC season string↔int validation (2025-99/2025-25/2025-2 → null, not silent-normalized), GAME_DATE→ISO source-effective anchor, traded-player tricodes, duplicate CONSUMED header (not just required) → incomplete, conflicting duplicate GAME_ID rows → fail-closed (order-independent), exact-dup rows collapsed + blank GAME_ID surfaced in diagnostics
npx tsx server/pregameTargets/ingestion/nbaFeatureBuilder.test.ts # Pregame Targets PR5 — feature builder: honest knownAt (=fetchedAt, never game date), missing/observed_zero/not_applicable distinct, known-before-valid rejection, structural validity, no line/price/EV/outcome key
npx tsx server/pregameTargets/ingestion/nbaPosteriorBuilder.test.ts # Pregame Targets PR5 — posterior fold via PR1 modules: deterministic/order-independent, included-game lineage, no-self-update, current+2-prior rollover (out-of-window weight 0), value-bearing only, same-game correction
npx tsx server/pregameTargets/ingestion/ingestionCoverage.test.ts # Pregame Targets PR5 — coverage: success → adapter_retrievable (live depth PENDING MEASUREMENT), failure → incomplete (never complete), prior season → historical knownAt unsupported
npx tsx server/pregameTargets/ingestion/nbaIngestionJob.test.ts # Pregame Targets PR5 — ingestion job (mock ports): audit-4 observation identity — idempotency is equality to the CURRENT HEAD (by knownAt), so A→B→A recurrence appends a NEW capture (A1←B←A2, A1/A2 same contentHash different capture id), true idempotency (same content later instant → no-op), historical reuse A→B→C→B (second B new), out-of-order older-knownAt → stale_observation (write nothing, no fork), same-knownAt different payload → conflicting_observation; ORCHESTRATOR identity firewall (mismatched season/label, unsupported type, invalid currentSeason/asOfDate throw BEFORE fetch); transport/incomplete write nothing; empty resultSet → incomplete; CONCURRENT seasons keep BOTH (no lost update); concurrency chronology (later commits first → older fails closed stale); atomic rollback; timestamp-policy metadata stamped
npx tsx server/pregameTargets/ingestion/nbaProviderBridge.test.ts # Pregame Targets PR5 — RAW provider→adapter bridge (the runner's real path, not getPlayerGameLogs): fetchRawNbaPlayerGameLog requests the EXACT season+seasonType (identity==request; Playoffs key ≠ Regular key), transport/http/json typed failures carry failedAt (never a successful fetchedAt), successful fetchedAt captured AFTER payload decode (delayed-fetch proof: fetchedAt > requestedAt and cannot precede payload resolution), verbatim payload so MIN/PTS null survive (never coerced to 0), a real 0 stays 0, omitted PTS column → missing, duplicate PTS column in the live payload → incomplete, parseSeasonType enum gate (runner exit 2)
npx tsx server/pregameTargets/ingestion/nbaIngestionErrors.test.ts # Pregame Targets PR5 — bounded/redacted failure records: a message embedding a connection string / provider-payload marker / auth bearer / DATABASE_URL is dropped (never leaks), benign message kept + bounded to 200 chars, non-Error throwables classified without throwing
npx tsx server/dbMigrations/pregameTargetsRawProvenancePersistence.test.ts # Pregame Targets PR5 — raw-snapshot audit-metadata + observation-chain schema bootstrap: additive ADD COLUMN IF NOT EXISTS self-heal (source_published_at/known_at_policy_version/supersedes_snapshot_id/semantic_source_key), partial supersedes index + (semantic_source_key, known_at) head index, no destructive SQL, idempotence, failure propagates
npx tsx server/pregameTargets/ingestion/nbaIngestionStorage.integration.test.ts # Pregame Targets PR5 — REAL-DB atomicity/recurrence/chronology (EXCLUDED_ENV without DATABASE_URL): post-capture-prep failure rolls back everything; A→B→A recurrence = three distinct captures (A1←B←A2 by knownAt) with replay before/between/after each transition; semantic key + timestamp-policy metadata survive the round trip (sourcePublishedAt null explicit); consecutive identical content → no-op; out-of-order older-knownAt → stale_observation (no fork, head not inverted); same-knownAt different payload → conflicting_observation; feature sourceId resolves to the exact capture; two concurrent same-player season ingests keep both seasons
npx tsx server/pregameTargets/ingestion/replayParity.test.ts # Pregame Targets PR5 — as-of replay over ingested rows: byte-equivalent reconstruction, honest knownAt (prediction before fetchedAt sees nothing), knownAt==predictionAt admissible, missing vs observed_zero survive replay, correction invisible to a replay before it
npx tsx server/pregameTargets/ingestion/pr5Isolation.test.ts # Pregame Targets PR5 — isolation + blindness: ingestion imports no other sport engine and no routes/persisted_plays/grading/analytics; runner uses only PR1 pregame_* storage; no line/price/EV/settlement token; ingested rows carry no forbidden key
npx tsx server/pregameTargets/ingestion/fixtures.test.ts # Pregame Targets PR5 — committed SYNTHETIC feasibility fixtures (cases.json, "synthetic":true): 13 cases (current/prior-1/prior-2/traded/team-change/missing/observed-zero/corrected/incomplete/reordered-headers/missing-required-header/duplicate-required-header/same-content-different-games) validated against the real adapter/feature-builder/coverage/identity
npx tsx shared/pregameTargets/canonicalEntities.test.ts # Pregame Targets PR6 — canonical NFL ids: "nfl" added to PREGAME_SPORTS (gsis-style native id build/parse/round-trip, team ids), MLB/NCAAB still rejected (not pregame sports), xfl still unknown_sport (fail-closed resolver unchanged)
npx tsx server/utils/access.test.ts # Pregame Targets PR6 — NFL entitlement mapping GATED OFF: fail-closed NFL_ENTITLEMENT_ENABLED flag (default off → hasNFL false for every real tier, others unchanged), pure tierMapsToNfl(elite), admin bypass unchanged, flag-on grants only elite
npx tsx server/pregameTargets/ingestion/nfl/nflCsvAdapter.test.ts # Pregame Targets PR6 — nflverse CSV adapter: RFC4180-ish parse (quotes/embedded commas/escaped quotes/CRLF), name-based columns, fail-closed on missing-required/duplicate-consumed header + wrong-width row + conflicting-duplicate-key, observed vs missing(blank/NA) vs observed_zero, blank-key dropped+surfaced, exact-dup collapsed, schedule parse + gameday→ISO
npx tsx server/pregameTargets/ingestion/nfl/nflFeatureBuilder.test.ts # Pregame Targets PR6 — NFL per-game feature builder: schedule-join anchor (home/away, validAt=gameday, canonical game id), missing vs observed_zero distinct, known-before-valid rejection, no-team/unresolved-anchor skipped with reasons, no line/price/EV/outcome key
npx tsx server/pregameTargets/ingestion/nfl/nflPosteriorBuilder.test.ts # Pregame Targets PR6 — NFL posterior fold via PR1 modules: deterministic/order-independent, value-bearing only, same-game correction (single lineage), current+2-prior rollover (out-of-window weight 0)
npx tsx server/pregameTargets/ingestion/nfl/nflIngestionCoverage.test.ts # Pregame Targets PR6 — NFL coverage: success → adapter_retrievable (live depth PENDING MEASUREMENT), failure → incomplete (never complete), prior season → historical knownAt unsupported
npx tsx server/pregameTargets/ingestion/nfl/nflIsolation.test.ts # Pregame Targets PR6 — NFL ingestion isolation: nfl/ modules import no other sport engine (incl. no nba* ingestion files) and no routes/persisted_plays/grading/analytics; no line/price/EV/settlement vocabulary in code
npx tsx server/pregameTargets/ingestion/nfl/nflIngestionJob.test.ts # Pregame Targets PR6 — NFL ingestion job (mock dataset store): one weekly file → MANY players' posteriors (multi-entity fold, no lost update); head-by-knownAt observation chain (A→B→A recurrence, true idempotency, stale, conflict); identity firewall (invalid season/currentSeason/asOfDate throw BEFORE fetch); schedule/weekly provider+parse failures write nothing; atomic rollback
npx tsx server/pregameTargets/ingestion/nfl/nflIngestionErrors.test.ts # Pregame Targets PR6 — NFL bounded/redacted failure records: connection string / provider-payload marker / auth bearer / DATABASE_URL dropped, benign message kept + bounded to 200 chars, non-Error throwables classified without throwing
npx tsx server/pregameTargets/ingestion/nfl/nflverseProvider.test.ts # Pregame Targets PR6 — nflverse provider URL construction against the FROZEN authoritative upstream paths (nflverse-data stats_player release asset + nfldata data/games.csv), post-decode fetchedAt honesty, typed transport/http/decode failures carrying failedAt (never a successful fetchedAt)
npx tsx server/pregameTargets/ingestion/nfl/fixtures.test.ts # Pregame Targets PR6 — committed SYNTHETIC feasibility fixtures (__fixtures__/cases.json, "synthetic":true): weekly parse / schedule multi-season filter / weekly→schedule join-by-game_id / coverage cases validated against the real adapter+feature-builder+coverage (normal, observed-zero, missing/NA, traded player, exact-dup collapsed, conflicting-dup, missing/duplicate header, wrong-width, season-mismatch, non-integer/out-of-range week, blank game_id, unresolved/contradictory join)
npx tsx server/pregameTargets/ingestion/nfl/nflIngestionStorage.integration.test.ts # Pregame Targets PR6 — REAL-DB dataset-scoped atomicity (EXCLUDED_ENV without DATABASE_URL): weekly+schedule+join = 3 immutable captures, feature sourceId resolves the join capture whose payload references BOTH input ids/hashes (both inputs recoverable), fold-throw atomic rollback, A→B→A recurrence chain by known_at, identical-content no-op, out-of-order stale, same-knownAt conflict, concurrent DIFFERENT-season same-player ingests keep BOTH posteriors (cross-season lock)
npx tsx shared/mlbPredictionLedger.test.ts # MLB Live Edge Stage B PR1 — all-lane prediction ledger contract: frozen/mutable partition exhaustive+disjoint, OVER/UNDER/push grading (never guesses on NaN), single-write settle/void, terminal-safety, frozen-field mutation guard (incl. signalScore as a no-authority research feature)
npx tsx server/mlb/stageB/predictionLedgerCapture.test.ts # MLB Live Edge Stage B PR1 — pure all-lane capture builder: official/watch/shadow all captured, home_runs excluded (HR Radar owns lifecycle), capture-gate off ⇒ none, fail-closed skip (unstamped lane/bad line/prob/side), field-mapping fidelity, calibrated-null preserved, statAtCapture respects currentStatKnown
npx tsx server/mlb/stageB/predictionLedgerSettlement.test.ts # MLB Live Edge Stage B PR1 — pure settlement decision: not-final ⇒ hold (never void on transient), final+stat ⇒ graded settle (0 is real, not void), DNP ⇒ immediate void, unresolvable ⇒ hold-then-terminal-void@48h, postponed/suspended ⇒ hold-then-void, terminal ⇒ defensive hold
npx tsx server/dbMigrations/mlbLanePredictionLedgerPersistence.test.ts # MLB Live Edge Stage B PR2 — prediction-ledger schema bootstrap: append-only PK, all-lane/prediction columns present with expected null-ness, idempotent IF-NOT-EXISTS-only, required indexes by name (incl. status/captured_at composite for the sweep), no destructive SQL, failure propagates
npx tsx server/mlb/stageB/predictionLedgerCaptureService.test.ts # MLB Live Edge Stage B PR3 — capture service (hot-path adapter): builds+appends fresh all-lane rows (HR excluded), capturedAt from injected clock, real version provenance stamped, NEVER throws when the storage append fails, empty/all-skipped ⇒ append not called
npx tsx server/mlb/stageB/predictionLedgerSettlementSweep.test.ts # MLB Live Edge Stage B PR3 — settlement sweep (injected fakes): one box fetch per game (dedup), final+stat ⇒ graded settle, DNP ⇒ void, young not-final ⇒ hold (never void on transient), old not-final ⇒ terminal void, fetch/settle failures counted not thrown, maxGamesPerSweep cap
npx tsx server/mlb/stageB/predictionLedgerSummary.test.ts # MLB Live Edge Stage B PR3 — pure ledger summary: per-lane status/result counts, hit rate = cashed/(cashed+missed) excluding push+void, coverage = resolved/total, empty lane/ledger ⇒ null hit rate (no divide-by-zero), overall aggregates all lanes
npx tsx shared/mlbCalibration.test.ts # MLB Live Edge Stage C PR1 — calibration artifact apply: piecewise-linear across bin centers, flat-extrapolated, MONOTONIC non-decreasing, clamped [0,100], empty bins ⇒ null (never identity), duplicate centers collapse to max (no divide-by-zero)
npx tsx server/mlb/stageC/calibrationMath.test.ts # MLB Live Edge Stage C PR1 — pure math: Brier/log-loss (clamped, no ±Inf), reliability bins (p===1 in last bin), shrinkage-to-prior, weighted isotonic PAV (fixes violations, always non-decreasing), ECE (well-calibrated≈0, overconfident≈40pp)
npx tsx server/mlb/stageC/fitCalibrator.test.ts # MLB Live Edge Stage C PR1 — fit from ledger: only settled cashed/missed observed (push/void/captured excluded), calibration IMPROVES Brier on a miscalibrated sample, monotonic bins, distinct 6am-ET SLATE dates (slateDateET rollover, not calendar date), in-sample honesty flag, per-market + lane-split segments, empty ⇒ no artifact
npx tsx server/mlb/stageC/calibratorPromotionGate.test.ts # MLB Live Edge Stage C PR1 — promotion gate (never applied): in-sample-only blocks, PARTIAL out-of-sample evidence ⇒ held_out_evidence_incomplete (no silent in-sample fallback — fail-closed), held-out all-criteria ⇒ ready, insufficient sample/dates, no-Brier-improvement, over-cap Brier/ECE, missing/negative forward ROI + unknown tier-monotonicity fail-closed
npx tsx server/dbMigrations/mlbCalibrationArtifactsPersistence.test.ts # MLB Live Edge Stage C PR2 — calibration-artifacts schema bootstrap: append-only PK, segment/artifact-jsonb NOT NULL, promotion_ready default false, idempotent IF-NOT-EXISTS-only, required indexes by name (incl. segment/built_at composite), no destructive SQL, failure propagates
npx tsx server/mlb/stageC/calibrationRunner.test.ts # MLB Live Edge Stage C PR2/PR3 — offline runner (injected fakes): fits+saves one artifact per segment, in-sample never promotion-ready, window bound, NEVER throws, latestArtifactPerSegment dedupe + PR3 promotion wiring (flag OFF ⇒ zero registry writes/refresh; flag ON ⇒ under-evidence active segment deactivated + registry refreshed; activationToInsertRow provenance)
npx tsx server/mlb/stageC/walkForwardEvaluation.test.ts # MLB Live Edge Stage C PR3 — out-of-sample walk-forward eval: forward-chaining (train strictly past, score never-seen validation slate), held-out Brier/ECE (calibration improves miscalibrated data), forward captured-price ROI gated on calibrated edge (null when no qualifying bet — honest unknown), validation set is a strict subset, per-segment split
npx tsx server/mlb/stageC/calibratorPromotion.test.ts # MLB Live Edge Stage C PR3 — pure auto-promotion planner: complete held-out evidence ⇒ ready; missing/undefined walk-forward ⇒ held_out_evidence_incomplete (never in-sample fallback); no-Brier-improvement/negative-ROI/non-monotonic block; flag OFF ⇒ empty plan; flag ON ⇒ passing activated, under-evidence active segment deactivated
npx tsx server/mlb/stageC/activeCalibratorRegistry.test.ts # MLB Live Edge Stage C PR3 — in-memory hot-path registry: TRIPLE fail-closed (flag OFF ⇒ null even loaded; out-of-support ⇒ null; unlisted segment ⇒ null), market:lane precedence over bare market, inactive rows excluded, NaN ⇒ null
npx tsx server/mlb/mlbSignalFinalizerCalibration.test.ts # MLB Live Edge Stage C PR3 — calibration flows through the finalizer ONLY when the flag is on + a compatible active calibrator is loaded (semantics flip to outcome_calibrated); raw engineProbability NEVER mutated; no-calibrator ⇒ raw_provisional; home_runs never calibrated
npx tsx server/dbMigrations/mlbActiveCalibratorsPersistence.test.ts # MLB Live Edge Stage C PR3 — active-calibrator registry schema bootstrap: segment PK, artifact jsonb + active flag + deactivation audit columns, idempotent IF-NOT-EXISTS-only, active index, no destructive SQL, failure propagates
```
...
### 3.10 MLB Live Edge Stage B — all-lane prediction ledger (research-only, PR1 foundation)
The Stage A safety-core (§3.2c) makes the **official** feed fail-closed but promotes no calibrator — production stays fail-closed/shadow by design. **Stage B** builds the private dataset a future **Stage C** offline calibrator needs: it records **every** MLB Live Edge prediction the finalizer emits — `official`, `watch`, **and** `shadow` — exactly as the model saw it, then grades each against the real final outcome.

- **Contract** (`shared/mlbPredictionLedger.ts`): `MlbLanePrediction` — a frozen capture record (identity/captured market state/model output incl. candidate & calibrated probability, no-vig edge, `edge_version`, finalized tier, `signalScore` **as a no-authority research feature only**, versions) plus a tiny mutable settlement surface (`status`/`settlementResult`/`finalStat`/`settledAt`/`voidReason`), guarded by `applyMlbLanePredictionLifecycleEvent` (throws on any frozen-field mutation, illegal transition, or patch to a terminal row) and `settleMlbLanePrediction`/`voidMlbLanePrediction` (single-write, graded against the row's own frozen side/line). `gradeMlbLanePredictionOutcome` mirrors the persisted grader: push on integer-line equality, cashed/missed per side, **throws rather than guessing** on a non-finite stat.
- **Pure capture builder** (`server/mlb/stageB/predictionLedgerCapture.ts`): maps a finalized `MLBQualifiedSignal` (any lane) → a frozen ledger row. Captures **all lanes** (deliberately NOT gated on `lane==="official"`), **excludes `home_runs`** (HR Radar owns its lifecycle), is **fail-closed** (skips unstamped-lane / non-finite line/prob rows), and honors `MLB_PREDICTION_CAPTURE_ENABLED_DEFAULT`. `capturedAtMs` is injected — pure/deterministic.
- **Pure settlement decision** (`server/mlb/stageB/predictionLedgerSettlement.ts`): `decideLanePredictionSettlement(pred, resolvedOutcome)` → hold/settle/void, reusing the persisted grader's conventions (not-final ⇒ hold, never void on transient; DNP ⇒ immediate void; unresolvable/postponed ⇒ hold-then-terminal-void@48h). The outcome is resolved by the caller — the future sweep reuses `fetchMlbBoxScore`/`getMlbStatValue` **read-only**.

- **Persistence (PR2)**: `mlbLanePredictions` (`shared/schema.ts`) — an additive, **append-only, private** table (PK `prediction_id`, unique per capture; `status`/`captured_at` composite index for the sweep) bootstrapped idempotently in `server/dbMigrations/mlbLanePredictionLedgerPersistence.ts` (wired fail-hard in `server/index.ts`, `CREATE TABLE IF NOT EXISTS` + indexes, no DROP). `IStorage` (`server/storage.ts`) exposes `appendMlbLanePredictions` (INSERT-only, `onConflictDoNothing` — a re-capture is a no-op, never an overwrite of a frozen row), `listPendingMlbLanePredictions` (captured, oldest-first), `getMlbLanePredictionsByGame`, and `settleMlbLanePrediction`/`voidMlbLanePrediction` — the two mutators run the contract's guarded pure function (throws on terminal/invalid) BEFORE a column-scoped UPDATE limited to the settlement columns, so frozen columns are never in a SET clause.

- **Capture wiring (PR3)**: `server/mlb/stageB/predictionLedgerCaptureService.ts`'s `captureAllLanesToStageB` is called **fire-and-forget** in `liveGameOrchestrator.ts` immediately after `autoPersistMLBSignals`, reading the **fresh all-lane `allSignals`** set (event-driven emissions — bounded, one capture per meaningful state change). It is fully `try/catch`-guarded and `.catch`-chained: it can NEVER throw into, block, or otherwise affect the tick. It stamps the real `MLB_FINALIZATION_VERSION`/`MLB_PRODUCTION_LANE_VERSION`/`MLB_GOLDMASTER_VERSION` and appends via `storage.appendMlbLanePredictions`. Tag: `[MLB_STAGE_B_CAPTURE]` (rate-limited) / `[MLB_STAGE_B_CAPTURE_ERROR]`.
- **Settlement sweep (PR3)**: `server/mlb/stageB/predictionLedgerSettlementSweep.ts`'s `runStageBSettlementSweep` (scheduled in `server/index.ts`: first run +4min, then every 5min) lists pending rows, groups by game, fetches ONE box score per game via `fetchMlbBoxScore` READ-ONLY, resolves each row's outcome (`buildMlbPlayerStats`/`getMlbStatValue`), decides via the pure `decideLanePredictionSettlement`, and applies through `storage.settleMlbLanePrediction`/`voidMlbLanePrediction`. Whole-sweep single-flight, bounded games/sweep, NEVER throws, holds (never voids) transient/not-final rows, terminal-voids genuinely abandoned ones. Tag: `[MLB_STAGE_B_SETTLE]` / `[MLB_STAGE_B_SETTLE_ERROR]`.
- **Admin read (PR3)**: `GET /api/admin/mlb/stage-b-ledger` (`requireAdmin`, `?days=N&limit=N`) → pure `summarizeStageBLedger` (per-lane capture/settlement counts + hit rate excl. push/void + coverage). Research surface only.

**Isolation is the feature (structurally enforced, same discipline as §3.7 shadow):** write-only to the Stage B store; **never** `persisted_plays`/ROI/W-L/public. This contract is **downstream measurement, never an engine input** (capturing a value can never change what the engine emitted); the sweep reuses the official outcome source strictly READ-ONLY and never calls `storage.settlePlay`. **PR1 (contract/builder/decision), PR2 (persistence), and PR3 (capture wiring + settlement sweep + admin read) are all landed.** The hot-path hook is purely additive (reads already-finalized signals, mutates nothing), so **no goldmaster re-baseline is required** — Stage B changes no engine math, probability, lane, or tier. Capture is gated by `MLB_PREDICTION_CAPTURE_ENABLED_DEFAULT` (`productionPolicy.ts`); public shadow surfacing stays off. Remaining Stage B work: nothing structural — future refinements (per-signal capture throttling, richer calibration analytics) build on this.

### 3.11 MLB Live Edge Stage C — offline calibrator + gated promotion (research-only, PR1–PR3)
Stage C is the **offline calibrator** that reads the Stage B ledger and fits the raw→calibrated probability mapping Stage A deliberately left `null` for. **Calibration is offline and GATED**: fitting an artifact NEVER promotes it. A calibrator affects engine output ONLY when a segment clears the promotion gate on **out-of-sample walk-forward evidence** AND the master switch `MLB_CALIBRATION_PROMOTION_ENABLED` is on — **default OFF**, so `mlbProductionLane` keeps `calibratedProbability = null` and production stays fail-closed/shadow until the flag is flipped.

- **Artifact contract** (`shared/mlbCalibration.ts`): `MlbCalibrationArtifact` — a versioned per-segment record of monotonic reliability bins (`center`/`empiricalRate`/`calibratedRate`) + `fitStats` (sample size, distinct ET slate dates, base rate, raw-vs-calibrated Brier/log-loss/ECE, `inSample` honesty flag). `applyCalibrator(artifact, rawProbPct)` is the pure, **monotonic, clamped** mapping a future promotion would call — piecewise-linear across bin centers, flat-extrapolated, `null` on empty bins (never an identity copy). Using it here does NOT wire it into the engine.
- **Math** (`server/mlb/stageC/calibrationMath.ts`): pure reliability binning, shrinkage-to-prior, weighted **isotonic pool-adjacent-violators** (monotonic), Brier/log-loss/ECE — generic over `{p,y}` so the same functions score raw, calibrated, and (later) held-out sets.
- **Fit** (`server/mlb/stageC/fitCalibrator.ts`): extracts settled **cashed/missed only** observations (push/void/captured excluded) from `MlbLanePrediction[]`, groups by segment (market, or lane-split), fits an artifact per segment with honest **in-sample** raw-vs-calibrated metrics + distinct ET slate dates (`toEtDateKey`). Pure, read-only over the rows.
- **Promotion gate** (`server/mlb/stageC/calibratorPromotionGate.ts`): pure `evaluateCalibratorPromotionReadiness` mirroring `productionPolicy.ts`'s single-owner `MlbPromotionThresholds` — **fail-closed**: blocks on in-sample-only, insufficient sample/slate-dates, calibrated Brier above cap or not better than raw, ECE above cap, and missing/negative forward ROI or unknown tier-monotonicity. **Checks only — never auto-applies** (same discipline as `moundV2PromotionGate.ts`).

- **Persistence + offline runner + admin (PR2)**: `mlbCalibrationArtifacts` (`shared/schema.ts`) — an additive, **append-only** history of fitted artifacts (PK `artifact_id = ${segment}:${builtAtMs}`; full `MlbCalibrationArtifact` in an `artifact` jsonb + flattened fit-stat columns + in-sample `promotion_ready`/`promotion_reasons`), bootstrapped idempotently in `server/dbMigrations/mlbCalibrationArtifactsPersistence.ts` (wired fail-hard in `server/index.ts`). `IStorage`: `saveMlbCalibrationArtifacts` (INSERT-only, `onConflictDoNothing`) + `listMlbCalibrationArtifacts`. `server/mlb/stageC/calibrationRunner.ts`'s `runCalibrationFit` is a scheduled OFFLINE background job (`server/index.ts`: +10min, then every 6h) that reads a window of the ledger READ-ONLY, fits per segment, evaluates in-sample readiness (always blocks — informational), and persists artifacts. NEVER throws; writes ONLY calibration artifacts; **never promotes.** Admin read: `GET /api/admin/mlb/calibration-artifacts` (`requireAdmin`, latest per segment or `?segment=` history). Tags: `[MLB_STAGE_C_CALIBRATION]` / `[MLB_STAGE_C_CALIBRATION_ERROR]`.

**Structurally isolated, offline-only (PR1–PR2).** Nothing under `server/mlb/stageC/` was imported by the engine/finalizer/lane/orchestrator through PR2; the runner is a background job that reads the ledger and writes only artifacts.

- **PR3 — gated calibrator promotion (landed, flag default OFF).** The runner now, after each offline fit, runs a **forward-chaining walk-forward evaluation** (`walkForwardEvaluation.ts` — train strictly on past slates, score on the never-seen validation slate; pooled held-out Brier/ECE, forward captured-price ROI gated on the calibrated edge, tier-monotonicity) and feeds that OUT-OF-SAMPLE evidence to `calibratorPromotionGate.ts`. `calibratorPromotion.ts` (pure) turns (artifacts + walk-forward + active set + flag) into an activate/deactivate plan — **empty when the flag is off**; a segment with no held-out evidence blocks as `held_out_evidence_incomplete` (never in-sample). Auto-promotion **auto-activates any segment that clears the gate and auto-deactivates ones that no longer qualify**, writing the durable `mlb_active_calibrators` registry (one row per segment, PK; `active` flag + deactivation audit — a row is kept, never deleted). The **in-memory `activeCalibratorRegistry.ts`** is the ONE hot-path read: a synchronous, pure `lookupCalibratedProbability` that is **TRIPLE fail-closed** — returns null unless (1) `MLB_CALIBRATION_PROMOTION_ENABLED` is on, (2) a compatible active calibrator exists for the segment, and (3) the raw prob is within the calibrator's fitted support (no extrapolation shipped). The finalizer's `computeLane` consults it before `evaluateMlbProductionLane`, stamping `calibratedProbability` and routing the calibrated value into lane/edge/tier **DECISIONS** — **the raw `engineProbability` is never mutated** (calibration is a decision input + provenance stamp only, per Hard Rule 5). HR is excluded. **Goldmaster bumped to v28** to document the new decision input (drift only appears once the flag is flipped for a promoted segment; with the flag off, output is byte-identical to v27). Admin: `GET /api/admin/mlb/active-calibrators` (registry + is-live signal), `POST /api/admin/mlb/active-calibrators/:segment/deactivate` (manual kill switch + registry reload), and an `promotion.isLive`/`activeSegments` block added to `GET /api/admin/mlb/calibration-artifacts`.

**PR1 (contract/math/fit/gate), PR2 (persistence + offline runner + admin), and PR3 (walk-forward + gated auto-promotion + registry + finalizer application + admin) are all landed.** The flag stays OFF by default — flipping `MLB_CALIBRATION_PROMOTION_ENABLED=true` is the explicit, human-reviewed go-live step. Do not flip it without evidence a segment clears the gate on out-of-sample data.
...
| MLB Live Edge Stage C — offline calibrator + gated promotion (research-only, PR1–PR3) | `shared/mlbCalibration.ts` (`MlbCalibrationArtifact` + monotonic/clamped `applyCalibrator` + `isWithinCalibratorSupport` + `MlbActiveCalibrator`/`MlbCalibratorPromotionSnapshot`), `server/mlb/stageC/calibrationMath.ts` (binning/shrinkage/isotonic-PAV/Brier/log-loss/ECE), `fitCalibrator.ts` (fit per segment from settled ledger rows), `walkForwardEvaluation.ts` (out-of-sample forward-chaining held-out metrics + forward ROI + tier-monotonicity), `calibratorPromotionGate.ts` (fail-closed readiness checker), `calibratorPromotion.ts` (pure activate/deactivate planner, empty when flag off), `activeCalibratorRegistry.ts` (in-memory cache-only hot-path lookup, triple fail-closed), `calibrationRunner.ts` (scheduled offline fit + walk-forward + gated auto-promotion), flag in `productionPolicy.ts` (`MLB_CALIBRATION_PROMOTION_ENABLED_DEFAULT=false` + `resolve`/`isMlbCalibrationPromotionEnabled`), application in `mlbSignalFinalizer.ts` `computeLane` (raw `engineProbability` never mutated), persistence `shared/schema.ts` (`mlbCalibrationArtifacts`, `mlbActiveCalibrators`) + `server/dbMigrations/{mlbCalibrationArtifactsPersistence,mlbActiveCalibratorsPersistence}.ts` + `server/storage.ts` (`saveMlbCalibrationArtifacts`/`listMlbCalibrationArtifacts`/`listLatestMlbCalibrationArtifactPerSegment`/`upsertMlbActiveCalibrator`/`deactivateMlbActiveCalibrator`/`listActiveMlbCalibrators`/`listAllMlbActiveCalibrators`), admin `GET /api/admin/mlb/calibration-artifacts` (+ `promotion.isLive`), `GET /api/admin/mlb/active-calibrators`, `POST /api/admin/mlb/active-calibrators/:segment/deactivate` — **flag default OFF**: production stays `calibratedProbability=null` until `MLB_CALIBRATION_PROMOTION_ENABLED=true` |
| MLB Live Edge Stage B — all-lane prediction ledger (research-only, PR1–PR3) | Contract `shared/mlbPredictionLedger.ts`; pure builder `server/mlb/stageB/predictionLedgerCapture.ts`; pure settlement decision `predictionLedgerSettlement.ts`; capture service (hot-path adapter) `predictionLedgerCaptureService.ts` (wired in `liveGameOrchestrator.ts` after `autoPersistMLBSignals`); settlement sweep `predictionLedgerSettlementSweep.ts` (scheduled in `server/index.ts`); pure summary `predictionLedgerSummary.ts` (`GET /api/admin/mlb/stage-b-ledger`); persistence `shared/schema.ts` (`mlbLanePredictions`) + `server/dbMigrations/mlbLanePredictionLedgerPersistence.ts` + `server/storage.ts` (`append`/`listPending`/`listRecent`/`getByGame`/`settle`/`void`) — write-only research store, never `persisted_plays`/ROI/public |
| Cross-Radar (Mound→Plate target suggestions) | `server/mlb/pregame/composition/moundPlateTargets.ts` (pure join/rank/dedupe builder + `isMoundCompositionEligible` — cr_high AND `status==="active"` AND `firstPitchLockEligible` AND gameStatus not live/final/postponed; assumes its Plate input is already eligibility-filtered), `composition/enrichMoundResponse.ts` (`isPlateCompositionEligible` layers the same pregame-only lifecycle check on top of Plate's canonical `isPublicPregameSignal` — that predicate alone is a visibility-*and-retention* gate that stays true for a locked/graded signal during a live/final/suspended game, which is correct for Plate's own product but not for this temporary pregame-only cross-reference; wraps an ALREADY-BUILT `MoundRadarResponse`, never invoked from inside `buildMoundResponse`), `composition/loadPregameCompositionContext.ts` (Plate snapshot loader via the existing non-blocking `peekRadarSnapshot` + `MOUND_PLATE_TARGET_SUGGESTIONS_ENABLED` rollout flag — `PlateCompositionState` distinguishes `disabled` from `missing`), `composition/composeMoundResponse.ts` (route-level orchestration: canonical `buildMoundResponse` called exactly once, outside the try/catch — a genuine Mound exception propagates, never mislabeled as a Plate failure; composition applied after, fails closed to `[]` on any Plate error, exactly ONE console.log call tagged `[MOUND_PLATE_COMPOSITION]` per response on every path — success, disabled, missing, date_mismatch, load_error, and an unexpected composition exception — never a separate console.warn/console.error; failures carry `failureKind`/`failureMessage` (capped at 200 chars) in that same record, never a raw `Error`/stack). `server/mlb/pregame/mound/types.ts` and `diagnostics.ts` are untouched by this feature — no Plate types, no Plate/composition imports, unchanged `buildMoundResponse` signature and output. Suggestion tier is `Exclude<PregamePowerTier, "track">`; client field is optional (`plateTargetSuggestions?:`) and always read via `?? []`. Client display logic (heading/score-label selection) lives in `client/src/lib/mlb/moundPlateTargetsDisplay.ts`, unit-tested without a render harness (this codebase's established client-testing convention) |
...
- **Stage B ledger (research):** `[MLB_STAGE_B_CAPTURE]` (rate-limited aggregate) / `[MLB_STAGE_B_CAPTURE_ERROR]`, `[MLB_STAGE_B_SETTLE]` (per-sweep summary) / `[MLB_STAGE_B_SETTLE_ERROR]` — capture/settlement are fire-and-forget and never break the tick
- **Stage C calibrator (research/offline):** `[MLB_STAGE_C_CALIBRATION]` (per-run fit + promotion summary incl. `promotionEnabled`/`ready`/`activated`/`deactivated`) / `[MLB_STAGE_C_CALIBRATION_ERROR]`, `[MLB_STAGE_C_REGISTRY]` (hot-path registry load) / `[MLB_STAGE_C_REGISTRY_ERROR]`, `[MLB_STAGE_C_PROMOTION]` (boot master-switch state) — promotion is flag-gated (`MLB_CALIBRATION_PROMOTION_ENABLED`, default OFF); nothing here mutates raw engine probability
...
| `GET /api/admin/mlb/stage-b-ledger` | MLB Live Edge Stage B ledger summary (research) — private all-lane prediction capture/settlement snapshot (`?days=N&limit=N`): per-lane counts, hit rate (excl. push/void), coverage. Read-only; never public/ROI |
| `GET /api/admin/mlb/calibration-artifacts` | MLB Live Edge Stage C calibration artifacts (research, offline) — latest fitted raw→calibrated artifact per segment (or `?segment=` history): fit stats + in-sample promotion-readiness reasons + a `promotion.isLive`/`activeSegments`/`enabled` block. Read-only |
| `GET /api/admin/mlb/active-calibrators` | MLB Live Edge Stage C PR3 active (promoted) calibrator registry — every registry row (active + deactivated, with activation provenance + promotion evidence) plus the in-memory hot-path snapshot (`isLive`, `loadedActiveSegments`). Read-only |
| `POST /api/admin/mlb/active-calibrators/:segment/deactivate` | MLB Live Edge Stage C PR3 manual calibrator kill switch — flips one segment's active calibrator off (+ optional `{reason}`) and reloads the hot-path registry. Never deletes; safe regardless of the master switch |

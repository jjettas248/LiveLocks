...
npx tsx server/pregameTargets/ingestion/nbaIngestionStorage.integration.test.ts # Pregame Targets PR5 — REAL-DB atomicity/idempotency/concurrency (EXCLUDED_ENV without DATABASE_URL): post-raw-insert failure rolls back everything, identical rerun no-op on the actual content-identity index, two concurrent same-player season ingests keep both seasons (advisory-lock read/fold/write)

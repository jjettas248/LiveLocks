...
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/plateHrV2FeaturesV3.test.ts # PR7A.2 plate_hr_v2_features_v3 contract (shadow/additive): V3 = V2 + pitcherDiscipline, extended contactOpportunity, zoneLocation reshaped to explicit unavailable record; CURRENT stays V2; strict rejects extra/missing groups; authorized projection strips market+zoneLocation; parseAuthorizedProjection routes V3
...
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/retrosheetDisciplineEvidence.test.ts # PR7A.2 retrosheet_discipline evidence-kind validator: single-actor (batter XOR pitcher) payload, closed keys, non-neg int counts, internal-consistency invariants (swings=whiffs+contacts, contacts=fouls+inPlay, twoStrikePa=K+survived, sub≤denominator), hardened provenance (present+unique gameIds, coverage window, EXACT attribution notice, closed floor null-reasons), resolved batter hand L/R only (no S), retrosheet-only provider
...
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/retrosheetDisciplineReadIntegrity.test.ts # PR7A.2 retrosheet_discipline strict training-read integrity: rejects unauthorized provider, non-verified_as_of, entity/actor mismatch, wrong batter id, schema/version mismatch, retrosheet evidence on a V1/V2 prediction; V3 recentContactForm without valid contact_events stays unreadable

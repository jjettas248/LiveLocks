...
npx tsx server/pregameTargets/ingestion/nbaGameLogAdapter.test.ts # Pregame Targets PR5 — adapter: incomplete/empty/malformed classified (never fabricated), observed vs missing (null stat) vs observed_zero distinct, season string↔int, GAME_DATE→ISO source-effective anchor, traded-player tricodes
...
npx tsx server/pregameTargets/ingestion/nbaIngestionJob.test.ts # Pregame Targets PR5 — ingestion job (mock ports): identical rerun no-op, correction → new immutable snapshot + posterior correction + prior retained, provider failure/incomplete writes nothing (no fabrication), incremental cross-season accumulation
...
npx tsx server/pregameTargets/ingestion/fixtures.test.ts # Pregame Targets PR5 — committed feasibility fixtures (cases.json): 9 cases (current/prior-1/prior-2/traded/team-change/missing/observed-zero/corrected/incomplete) validated against the real adapter/feature-builder/coverage/identity
```

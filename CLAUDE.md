...
npx tsx server/pregameTargets/ingestion/rawSnapshotIdentity.test.ts # Pregame Targets PR5 — raw-snapshot identity: deterministic content hash + snapshotId, identical content → same id (idempotent no-op), correction → new id, supersession classification
...
npx tsx server/pregameTargets/ingestion/nbaIngestionJob.test.ts # Pregame Targets PR5 — ingestion job (mock ports): identical rerun no-op, correction → new immutable snapshot + posterior correction + prior retained, transport failure/incomplete writes nothing (no fabrication), empty resultSet → incomplete (not provider_failure), incremental cross-season accumulation, CONCURRENT same-player season ingests keep BOTH seasons (fold-under-lock, no lost update), ORCHESTRATOR identity firewall (mismatched season/label, unsupported season type, invalid currentSeason/asOfDate all throw IngestInvocationError BEFORE fetch), timestamp-policy metadata stamped on the raw row, correction lineage A←B←C (first capture has no predecessor; identical rerun no lineage change)
...
npx tsx server/dbMigrations/pregameTargetsRawProvenancePersistence.test.ts # Pregame Targets PR5 — raw-snapshot audit-metadata + correction-lineage schema bootstrap: additive ADD COLUMN IF NOT EXISTS self-heal (source_published_at/known_at_policy_version/supersedes_snapshot_id), partial supersedes index, no destructive SQL, idempotence, failure propagates
npx tsx server/pregameTargets/ingestion/nbaIngestionStorage.integration.test.ts # Pregame Targets PR5 — REAL-DB atomicity/idempotency/concurrency/lineage (EXCLUDED_ENV without DATABASE_URL): post-raw-insert failure rolls back everything, identical rerun no-op on the actual content-identity index, two concurrent same-player season ingests keep both seasons (advisory-lock read/fold/write), timestamp-policy metadata survives the round trip (sourcePublishedAt null explicit), correction chain A←B←C resolved under the lock, replay before/after a correction, feature sourceId resolves to the immutable raw snapshot + policy version, concurrent corrections form one linear chain (no fork)

...
npx tsx server/mlb/stageC/calibrationRunner.test.ts # MLB Live Edge Stage C PR2 — offline runner (injected fakes): fits+saves one artifact per segment, in-sample never promotion-ready (reasons incl in_sample_only), numeric fit-stats stored as strings + full artifact jsonb, window bound applied to the ledger read, NEVER throws on save failure, empty ledger ⇒ no save, latestArtifactPerSegment dedupe
```
...
### 3.11 MLB Live Edge Stage C — offline calibrator (research-only, PR1 foundation)
Stage C is the **offline calibrator** that reads the Stage B ledger and fits the raw→calibrated probability mapping Stage A deliberately left `null` for. **Calibration is offline and GATED**: fitting an artifact NEVER promotes it. Nothing in the live engine reads a calibrator until an explicit, human-reviewed promotion step wires it in — which does not exist yet — so `mlbProductionLane` keeps `calibratedProbability = null` and production stays fail-closed/shadow.
...
**Structurally isolated, offline-only.** Nothing under `server/mlb/stageC/` is imported by the engine/finalizer/lane/orchestrator; the runner is a background job that reads the ledger and writes only artifacts. **PR1 (contract/math/fit/gate) and PR2 (persistence + offline runner + admin) are landed.** The remaining work is the still-gated **promotion wiring (PR3)** — only after held-out/walk-forward evidence clears `calibratorPromotionGate.ts` may a calibrator be wired into `mlbProductionLane` (so it stops returning `calibratedProbability = null` for that segment). No goldmaster bump (Stage C changes no engine math). Do not wire a calibrator into the engine before the gate passes on out-of-sample evidence.
...
| MLB Live Edge Stage C — offline calibrator (research-only, PR1–PR2) | `shared/mlbCalibration.ts` (`MlbCalibrationArtifact` + monotonic/clamped `applyCalibrator` — not engine-wired), `server/mlb/stageC/calibrationMath.ts` (binning/shrinkage/isotonic-PAV/Brier/log-loss/ECE), `fitCalibrator.ts` (fit per segment from settled ledger rows), `calibratorPromotionGate.ts` (fail-closed readiness checker, never applied), `calibrationRunner.ts` (scheduled offline fit+persist), persistence `shared/schema.ts` (`mlbCalibrationArtifacts`) + `server/dbMigrations/mlbCalibrationArtifactsPersistence.ts` + `server/storage.ts` (`saveMlbCalibrationArtifacts`/`listMlbCalibrationArtifacts`), admin `GET /api/admin/mlb/calibration-artifacts` — offline/gated; production stays `calibratedProbability=null` until a human-reviewed promotion |
...
| `GET /api/admin/mlb/calibration-artifacts` | MLB Live Edge Stage C calibration artifacts (research, offline) — latest fitted raw→calibrated artifact per segment (or `?segment=` history): fit stats + in-sample promotion-readiness reasons. Read-only; nothing here promotes a calibrator |

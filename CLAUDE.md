...
**Isolation is the feature (structurally enforced, same discipline as §3.7 shadow):** write-only to the Stage B store; **never** `persisted_plays`/ROI/W-L/public. This contract is **downstream measurement, never an engine input** (capturing a value can never change what the engine emitted). **PR1 (contract/builder/decision) and PR2 (persistence) are landed; there is still NO hot-path wiring and NO settlement sweep — nothing writes to the table yet.** That is **PR3**: fire-and-forget capture at `liveGameOrchestrator.ts` next to `autoPersistMLBSignals` (reading the all-lane `merged.allSignals`), an interest-driven settlement sweep reusing `fetchMlbBoxScore`/`getMlbStatValue` READ-ONLY + the mound-V2 backoff-policy shape, and an admin read surface. Do not wire Stage B into a hot path or surface it publicly before PR3.
...
| MLB Live Edge Stage B — all-lane prediction ledger (research-only, PR1–PR2) | `shared/mlbPredictionLedger.ts` (frozen contract + guarded mutator + `gradeMlbLanePredictionOutcome`/`settleMlbLanePrediction`/`voidMlbLanePrediction`), `server/mlb/stageB/predictionLedgerCapture.ts` (pure all-lane capture builder), `server/mlb/stageB/predictionLedgerSettlement.ts` (pure hold/settle/void decision), `shared/schema.ts` (`mlbLanePredictions`), `server/dbMigrations/mlbLanePredictionLedgerPersistence.ts` (idempotent bootstrap), `server/storage.ts` (`appendMlbLanePredictions`/`listPendingMlbLanePredictions`/`getMlbLanePredictionsByGame`/`settleMlbLanePrediction`/`voidMlbLanePrediction`) — write-only research store, never `persisted_plays`/ROI/public; no hot-path capture/sweep wiring yet (PR3) |

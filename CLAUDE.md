...
npx tsx server/mlb/pregamePowerRadar/hrProbabilityV2/retrosheetDisciplineAdapter.test.ts # PR7A.3 pure/deterministic Retrosheet normalization adapter (cwevent raw rows + caller provenance → canonical PA facts → batter/pitcher RetrosheetDisciplineEvidencePayload + contactOpportunity/pitcherDiscipline V3 fragments + retrosheetDiscipline dataQuality): 8 fixtures as executable adapter tests + mutations — interrupted-PA single-terminal-row/marker-strip-once/no-double-count, responsible-batter substitution (PA/K to RESP_BAT_ID), unknown-handedness withholds only the affected split, fail-closed on duplicate terminal PA / count-arithmetic mismatch / unsupported pitch token / bad attribution notice / duplicate gameIds; floor-nulled derived rates with reasons (coverage>=0.90, batter>=150 PA, pitcher>=300 BF, split>=75 PA / >=150 BF) with raw counts always retained; pitcherCalledStrikeRatePct=calledStrikes/pitches (calledStrikes>BF valid); no legacy vsHand/batterHand keys; chase/zoneContact stay null; byte-deterministic output (row-order invariant); revalidated through merged validateRetrosheetDisciplinePayload + V3 schemas

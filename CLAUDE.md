...
**Isolation is the feature (structurally enforced, same discipline as §3.7 shadow):** write-only to the Stage B store; **never** `persisted_plays`/ROI/W-L/public. This contract is **downstream measurement, never an engine input** (capturing a value can never change what the engine emitted). **PR1 is the isolated, fully-tested foundation with NO persistence, NO hot-path wiring, and NO settlement sweep yet** — those are PR2 (schema/migration/storage) and PR3 (fire-and-forget capture at `liveGameOrchestrator.ts` next to `autoPersistMLBSignals`, reading the all-lane `merged.allSignals`; interest-driven settlement sweep reusing the mound-V2 backoff-policy shape + admin read surface). Do not wire Stage B into a hot path or surface it publicly before those PRs.
...
| MLB Live Edge Stage B — all-lane prediction ledger (research-only, PR1) | `shared/mlbPredictionLedger.ts` (frozen contract + guarded mutator + `gradeMlbLanePredictionOutcome`/`settleMlbLanePrediction`/`voidMlbLanePrediction`), `server/mlb/stageB/predictionLedgerCapture.ts` (pure all-lane capture builder), `server/mlb/stageB/predictionLedgerSettlement.ts` (pure hold/settle/void decision) — write-only research store, never `persisted_plays`/ROI/public; no persistence/wiring yet |

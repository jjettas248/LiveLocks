...
npx tsx server/mlb/stageC/fitCalibrator.test.ts # MLB Live Edge Stage C PR1 — fit from ledger: only settled cashed/missed observed (push/void/captured excluded), calibration IMPROVES Brier on a miscalibrated sample, monotonic bins, distinct ET slate dates, in-sample honesty flag, per-market + lane-split segments, empty ⇒ no artifact
npx tsx server/mlb/stageC/calibratorPromotionGate.test.ts # MLB Live Edge Stage C PR1 — promotion gate (never applied): in-sample-only blocks, held-out all-criteria ⇒ ready, insufficient sample/dates, no-Brier-improvement, over-cap Brier/ECE, missing/negative forward ROI + unknown tier-monotonicity fail-closed

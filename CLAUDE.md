...
**Structurally isolated, offline-only.** Nothing under `server/mlb/stageC/` is imported by the engine/finalizer/lane/orchestrator; it operates on arrays passed in (no DB queries, no writes) and produces artifacts only. **PR1 is the isolated, fully-tested foundation** (contract + math + fit + promotion gate). Later PRs: an offline runner (read ledger → fit → persist artifacts) + admin surface (PR2), and the still-gated promotion wiring (PR3, only after held-out/walk-forward evidence clears the gate). Do not wire a calibrator into `mlbProductionLane` before the gate passes on out-of-sample evidence.
...
| MLB Live Edge Stage C — offline calibrator (research-only, PR1) | `shared/mlbCalibration.ts` (`MlbCalibrationArtifact` + monotonic/clamped `applyCalibrator` — not engine-wired), `server/mlb/stageC/calibrationMath.ts` (binning/shrinkage/isotonic-PAV/Brier/log-loss/ECE), `fitCalibrator.ts` (fit per segment from settled ledger rows, in-sample metrics), `calibratorPromotionGate.ts` (fail-closed readiness checker, never applied) — offline/gated; production stays `calibratedProbability=null` until a human-reviewed promotion |

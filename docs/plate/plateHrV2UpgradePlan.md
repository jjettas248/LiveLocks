...
> **Current implementation head:** `f5d139b` (PR0–PR4.3.2 landed on `claude/plate-hr-engine-upgrades-ispzmm`, PR #151). **PR5** (stabilized recent-contact features, Deps: PR4.3) is next — unblocked now that PR4.3.2's V2 suites, champion lock (78/78), migration tests, and typecheck are green. This plan is committed at `docs/plate/plateHrV2UpgradePlan.md` as the canonical, verifiable implementation authority.
...
**PR5 — Stabilized recent-contact features.** *Deps:* **PR4.3** (hardened + read-verified statistics). *Obj:* EWMA 25–50 BBE + EV90 + pulled-air isolation; 15-BBE capped. *Files:* new `recentContactForm.ts`, `dataPullService.ts`, contract. *Tests:* HR count can't fire; 15-BBE regressed; 25–50 BBE > spike; missing bat speed tolerated. *Rollback:* shadow-only. *Accept:* invariants green.
...
5a. **PR4.1 / PR4.2 / PR4.3 / PR4.3.1 / PR4.3.2** — sufficient-stat correctness → write-side evidence hardening → **training-read integrity** (blank-safe parser, honest provenance, one canonical-object source-id used at write+read, strict batch training-reader verifying prediction+source hashes/point-in-time/uniqueness/sort, closed nested allowlist, no `{}` manufacture) → **read-integrity hardening** (`reconstructed` from `fetchedAt`, typed non-empty payloads at write+read, never-throw runtime-parse of untrusted DTOs, triple id-equality, sparse-array rejection) → **provenance matrix + payload-domain validation** (`availabilitySource` enum, unparseable-timestamp rejection, per-source timestamp law, reconstructed-only-verified-as-of, null-as-absence, nonneg-int counts + monotonic count law + percentile domains). **Blocks PR5+.**

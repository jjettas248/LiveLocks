...
> **Current implementation head:** `feffcb9` (PR0–PR6.2 on `claude/plate-hr-engine-upgrades-ispzmm`, PR #151; the branch is synchronized with current `main`). PR6 + PR6.1 are APPROVED (landed shadow-only). **PR6.2** (as-of starter/bullpen PA-path forward-capture prerequisite) is implemented shadow-only and **pending reviewer approval**. **PR7** (contact-aware zone term) stays BLOCKED on the PR2 zone spike (Savant location fields still UNVERIFIED — a `ZONE GATE: GO` in production is required). Latest gate: PR6.2 suites (`starterBullpenPaPath` 47 + 8 binding cases → snapshots 174) + full V2 (18 suites), champion 78/78, isolation 77/77, slate-invariance 39/39, ISO-tag 42, math `starterBullpenPath` 55 + `scoreRecentContactForm` 13, migrations (77 + 55), and typecheck all green — with main's `isoPopulationAuditGate` 22/22 and `goldenFixtures` 24/24.
...
**PR6.2 — As-of starter/bullpen PA-path forward-capture prerequisite (pre-PR8, LANDED, shadow-only).** *Obj:* capture the exposure evidence the corrected joint PA-path needs, as-of and fail-closed, so PR8 has real training evidence (no-backfill). *Fixes/build:*

...
> **Current implementation head:** `3f93cb2` (PR0–PR5 landed on `claude/plate-hr-engine-upgrades-ispzmm`, PR #151). **PR6** (corrected starter/bullpen joint PA-path probability, Deps: PR5) is next. PR5's suites (recentContactForm 20 + full V2), champion lock (78/78), migration tests, and typecheck are green. This plan is committed at `docs/plate/plateHrV2UpgradePlan.md` as the canonical, verifiable implementation authority.

...
> **Current implementation head:** `42d339a` (PR0–PR6 landed on `claude/plate-hr-engine-upgrades-ispzmm`, PR #151; the branch is synchronized with current `main`). **PR7** (contact-aware zone term, only if the PR2 zone spike passes) is next. PR6 (corrected starter/bullpen joint PA-path probability) was approved and landed shadow-only. Latest gate: PR6 suites (`starterBullpenPath` 35 + `scoreRecentContactForm` 13) + existing math suites unchanged, snapshot contract 166 + full V2 17 suites, champion (78/78), isolation (77/77), slate-invariance (39/39), ISO-tag (42), migrations (persistence 77 + snapshot persistence 55), and typecheck all green — together with main's `isoPopulationAuditGate` (22/22) and `goldenFixtures` (24/24). This plan is committed at `docs/plate/plateHrV2UpgradePlan.md` as the canonical, verifiable implementation authority. **Note:** the branch has been merged with `main`; PR0's display-layer Elite-ISO suppression was **superseded** by PR #152's canonical true-ISO gate (a different, now-authoritative implementation of the same intent) — the branch has since been kept in sync through subsequent `main` merges (latest `9035333`) with the champion (78/78) and slate-invariance (39/39) locks green.

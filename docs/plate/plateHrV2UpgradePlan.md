...
> **Current implementation head:** `feffcb9` (PR0–PR6.2 committed on the feature branch `claude/plate-hr-engine-upgrades-ispzmm`, PR #151; synchronized with current `main`). **Nothing is merged to `main` yet — PR #151 is open.** PR6 + PR6.1 are reviewer-APPROVED (shadow-only, pending merge). **PR6.2** (as-of starter/bullpen PA-path forward-capture prerequisite) is implemented + pushed and **pending review** — not merged, not "landed". **PR7** (contact-aware zone term) stays BLOCKED on the PR2 zone spike (Savant location fields still UNVERIFIED — a `ZONE GATE: GO` in production is required). Latest gate: PR6.2 suites (`starterBullpenPaPath` 47 + 8 binding cases → snapshots 174) + full V2 (18 suites), champion 78/78, isolation 77/77, slate-invariance 39/39, ISO-tag 42, math `starterBullpenPath` 55 + `scoreRecentContactForm` 13, migrations (77 + 55), and typecheck all green — with main's `isoPopulationAuditGate` 22/22 and `goldenFixtures` 24/24.
...
**PR5.3 — Bind contact evidence to the batter + strict nested projections (review correction, LANDED).** *Obj:* close two residual training-integrity bypasses. *Fixes:*
- **Batter/entity/provider binding:** the reader now requires the single contact_events source to satisfy `entityType === "batter"`, `entityId === prediction.batterId`, and `provider ∈ CONTACT_EVENTS_PROVIDERS` (allowlist: `mlb_stats_live`) — a coherently-rehashed source for a different batter/entity/provider can no longer attach.
- **Strict nested projections:** every nested feature-group schema is `.strict()` (all groups + the pitch-family leaves + `recentContactForm` + `dataQuality`; the authorized `extra` records unchanged), so an unauthorized nested field (e.g. `batterPower.unversionedFeature`) fails `parseAuthorizedProjection()` instead of being silently stripped and riding through a rehashed envelope.
*Tests:* wrong-batter / wrong-entity-type / unauthorized-provider (each coherently rehashed) rejected; unauthorized nested field rejected for V1 and V2. *Deps:* PR5.2. *Rollback:* shadow-only. *Accept:* full V2 suite + champion 78/78 + isolation 77/77 + slate-invariance 39/39 + ISO-tag 42 + migrations + tsc (met).

**PR5.3.1 — parkWeatherSpray strict + schema-matrix nested-field regression (review correction, LANDED).** *Obj:* close the one schema group PR5.3 missed. *Fixes:*
- **`plateHrV2ParkWeatherSprayFeaturesSchema` is now `.strict()`** — PR5.3's bulk strict-conversion skipped it because its `extra:` line carried a trailing comment, so a coherently-rehashed `derivedFeatures` with `parkWeatherSpray.unauthorizedFeature` still passed. All 14 nested feature-group schemas (+ the pitch-family leaf) are now strict (18 `.strict()` total in the contract).
- **Schema-matrix regression:** `runMatrix(version, projection)` iterates every authorized nested-object group key, injects an unauthorized property (`{...group, __unauthorized__: 123}`), coherently rehashes the envelope, and asserts the real reader rejects it with a `derived_projection*` reason — run for both V2 (`buildProjection()`) and V1 (`buildV1Projection()`), plus a pitch-family leaf and explicit coherently-rehashed `parkWeatherSpray` cases for both versions. No group can be missed mechanically again.
*Tests:* parkWeatherSpray unauthorized field rejected (V1 + V2, coherently rehashed); schema-matrix over every group; pitch-family leaf. *Deps:* PR5.3. *Rollback:* shadow-only. *Accept:* full V2 suite + champion 78/78 + isolation 77/77 + slate-invariance 39/39 + ISO-tag 42 + migrations + tsc (met). *Post-merge:* branch synchronized with current `main` (merge `9035333`); full gate re-verified green incl. main's `isoPopulationAuditGate` 22/22 and `goldenFixtures` 24/24.

**PR6 — Corrected starter/bullpen joint PA-path probability (APPROVED — landed shadow-only; corrected by PR6.1).** *Obj:* joint game-HR expectation + per-segment decomposition (§10); opener-aware PA-path; additive fitted-ready terms. *As-built:* the correction is **additive** — the legacy single-per-PA-rate fields (`matchupAdjustedHrPerPa`, `calibratedGameHrProbability`) stay byte-stable (existing math suites unchanged) and the corrected joint model is added alongside as new machinery + result fields. Per-segment rates: `p_s` (vs starter) folds in the starter opponent terms (pitcher vulnerability, starter pitch-mix, starter zone); `p_b` (vs bullpen) folds in ONLY expected-bullpen vulnerability; BOTH share hitter/recent-form/park. **Lineup drives PA VOLUME (the joint path), not the rate; market is excluded from probability entirely** (a genuine correction over the legacy rate, which folded both in). Exposure is applied **exactly once** in the PA-path (`scoreBullpenVulnerability` carries no exposure multiplier — the §11 double-count fix). `estimatePregamePaPath` builds the joint `(n_s, n_b)` distribution (opener-aware; corrected in PR6.1 — see below — so that no exposure evidence yields an UNAVAILABLE path, never a fabricated all-starter default). `jointGameHrProbability` = `1 − Σ P(n_s,n_b)(1−p_s)^{n_s}(1−p_b)^{n_b}`. `scoreRecentContactForm` is a new batter-intrinsic term from the PR5 stabilized features (EV/EV90/air%/barrel% only — **no HR-count surface**), reliability-shrunk by effective BBE, entering both segments; it is **not** wired into `toPregameMathInputs` (isolation preserved). *Files:* new `math/estimatePregamePaPath.ts`, new `math/scoreRecentContactForm.ts` (+ test), edited `math/gameHrProbability.ts` (`jointGameHrProbability`), `math/scoreStarterBullpenPath.ts` (`scoreBullpenVulnerability`), `math/buildPregameHrPerPa.ts` (`buildSegmentedHrPerPa`), `math/scorePitchTypeInteraction.ts` (starter-segment role note), `math/mathTypes.ts` (segment/joint contracts + optional `recentContactForm`), `math/mathDiagnostics.ts` (additive joint fields), new `math/starterBullpenPath.test.ts`. *Tests:* `starterBullpenPath.test` (enumeration parity, monotone in p_s/p_b/total-PA, starter-only-never-in-p_b, hitter/form/park-always-in-p_b, exposure-not-double-counted, bounded/finite) + `scoreRecentContactForm.test` (13). *Deps:* PR5 (which carries PR4/PR4.3). *Rollback:* shadow-only. *Accept:* property tests green + existing math suites unchanged + champion 78/78 + isolation 77/77 + slate-invariance 39/39 + ISO-tag 42 + hrProbabilityV2 (17 suites) + migrations + tsc (met).

**PR6.1 — PA-path exposure correctness + market isolation (review correction, APPROVED, LANDED, shadow-only).** *Obj:* fix two PA-path correctness defects and one market-isolation leak in PR6 (the joint formula + segment decomposition were confirmed sound). *Fixes:*
- **Missing exposure is no longer fabricated as 100% starter.** PR6 routed all PA through `p_s` when both projections were null (`allStarterPath`) — not neutral when starter terms are nonzero, and exactly the current real-capture shape (`buildPregamePowerRadar.ts` writes both projections + both bullpen metrics null). Now, with NO exposure evidence (no projected split AND no opener signal), `estimatePregamePaPath` returns an **UNAVAILABLE** path (`available:false`, empty `joint`, reason `missing_pa_path`); `jointGameHrProbability` returns **null** and `runPregameMathModel` nulls the joint fields + emits `missing_pa_path`. No-backfill means missing exposure must be **captured, never assumed**.
- **Both projected-PA fields move the path.** PR6 ignored `projectedPaVsBullpen` whenever `projectedPaVsStarter` was present. Now `starterShare = s/(s+b)`; when only one projection exists the other is inferred from the total-PA mean; boundary shares collapse cleanly to all-starter / all-bullpen. The **opener signal independently** moves mass to the bullpen (discounts the share when a split exists; frozen opener prior when no split exists).
- **Market isolation.** Both PA estimators consumed `lineupOpportunity.teamImpliedRuns` (a market-derived total) — a violation of the odds-excluded-from-probability rule. The joint path's total-PA marginal now uses **batting-order slot only**; `teamImpliedRuns` is excluded from `p_s`/`p_b`/PA-path/joint prob (legacy single-path diagnostic unchanged; type doc updated).
- **Capture prerequisite (tracked, not this PR):** the workload/removal + bullpen-vulnerability evidence the joint path needs must be **captured before PR8** (PR3-capture extension) — because of no-backfill, rows lacking it now correctly report `missing_pa_path` rather than a fabricated probability.
*Tests:* new regressions — bullpen-projection-only change, starter-projection-only change, opener-flag-only change, opener-with-no-split, unavailable-path + `missing_pa_path` (incl. the end-to-end real-capture shape), market-invariance (odds + team total cannot alter `p_s`/`p_b`/path/joint prob); plus the PR6 property block. *Deps:* PR6. *Rollback:* shadow-only. *Accept:* `starterBullpenPath.test` (55) + `scoreRecentContactForm.test` (13) + existing math suites unchanged + champion 78/78 + isolation 77/77 + slate-invariance 39/39 + ISO-tag 42 + hrProbabilityV2 (17 suites) + migrations + tsc (met).

**PR6.2 — As-of starter/bullpen PA-path forward-capture prerequisite (pre-PR8, IMPLEMENTED — pushed to PR #151, pending review; NOT merged; shadow-only).** *Obj:* capture the exposure evidence the corrected joint PA-path needs, as-of and fail-closed, so PR8 has real training evidence (no-backfill). *Fixes/build:*
- **Frozen as-of SOURCE contract** (`hrProbabilityV2/starterBullpenPaPath.ts`, pure) for the five inputs the reviewer named — starter workload/removal, opener classification, a **NON-market** projected-PA basis, bullpen composition/availability, and bullpen vulnerability + sample. `deriveStarterBullpenPaPath()` derives the projections + confidence + `missingReasons` **deterministically** and **fail-closed**: a missing source yields a null projection + explicit reason (never a league/generic default), the opener flag stays null when unknown (never a `false` default), and there is **no market/odds field anywhere** in the contract (PA volume comes from the non-market `expectedTotalPa`).
- **Content-addressed `starter_bullpen` evidence** (`starterBullpenPaPathEvidence.ts`) capturing the raw sources + the derived projection with provider / timestamps / entity binding / content hash; evidence is emitted **only** when the projection carries usable exposure (else the PA-path stays `missing_pa_path`).
- **Reader integrity** (`plateHrV2Snapshots.ts`): new `starter_bullpen` evidence kind (point-in-time rule = lineup/probable, `availableAt ≤ predictionAsOf ≤ firstPitch`); a strict payload validator (closed fields + the stored projection must **re-derive from its own sources**); and a read-time binding — a prediction whose `starterBullpen` group carries exposure must have exactly one **batter-bound, authorized-provider, version-matched** `starter_bullpen` source whose payload re-derives to the stored group; a no-exposure group must carry none. DB `evidence_kind` is free text, so **no migration** is needed (like `contact_events`).
- **Capture wiring** (`buildPregamePowerRadar.ts`, behind `PLATE_HR_V2_FORWARD_CAPTURE_ENABLED`): assembles the as-of source bundle from data available at build time and emits the evidence. **Fail-closed today** — the live starter-workload/bullpen fetchers are not wired yet, so the bundle is honestly null → no exposure → `missing_pa_path` (never fabricated). The opener signal is captured in the evidence payload (the strict derived group has no `isOpenerLikely` key). **No champion/public wiring; no market inputs.** *(Remaining data-plumbing — a SEPARATE, explicitly-authorized PR, not started: per the reviewer-locked sequence, first capture representative live MLB Stats workload/bullpen responses, freeze their normalization fixtures, then implement the narrow adapter against those fixtures — flowing through this frozen contract unchanged. No scaffolding is written ahead of inspecting the real payloads, to avoid another inactive surface that could drift. Until then, starterBullpen is EXCLUDED from fitting (PR8), champion comparison, and public output, and capture resolves to `missing_pa_path`.)*
*Tests:* `starterBullpenPaPath.test` (47 — deterministic derivation, fail-closed/no-default, partial-source degradation, non-market PA basis, evidence round-trip + provenance, as-of/no-lookahead eligibility, forged/malformed payload) + 8 read-time binding cases in `plateHrV2Snapshots.test` (174 total: valid admit, missing evidence, unexpected evidence, wrong batter, unauthorized provider, group≠evidence mismatch, forged payload). *Deps:* PR6.1. *Rollback:* shadow-only. *Accept:* full V2 suite (18) + champion 78/78 + isolation 77/77 + slate-invariance 39/39 + ISO-tag 42 + migrations + tsc (met).

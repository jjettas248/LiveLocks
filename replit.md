...
- **MLB UI Architecture**: Signal-first compact design — game cards are single-row (teams + score/time + pitchers + signal badge), BatterCard shows inline contact stats (EV, xBA, Hard%) only when data exists, GameDetailView groups signals by player+direction with PRIMARY/SECONDARY play labels, ABOutcomePill shows pitch type/speed and exit velocity. Pitch info (pitchType, pitchSpeed) flows from Savant BIP data through `priorABResults[]` to frontend display. Signal state machine classifies cards as ACTIONABLE/ALREADY_HIT/WATCHLIST/STALE with distinct visual treatments (emerald for hit, faded for stale, muted for watchlist). Signal cards in Live Feed are clickable and route to game → player → market → calculator. MLB bet slip floats at bottom-right for accumulating picks with copy/DraftKings export. Pitcher arsenal (4-seam, slider, curve, etc.) displayed on signal cards with attackability tags. UNDER accent is blue (#3b82f6) everywhere. Cold-label mislabeling fixed: players with HR or strong contact (2+ hard hits ≥95mph) are never classified as COLD.

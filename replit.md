...
- **MLB Live Monitoring Stability**: All polling queries (live-games, edge-feed, HR radar, alerts, box score, NBA games/signals) use `placeholderData: (prev) => prev` to prevent UI flashing during refetch cycles. Server-side signal freshness threshold relaxed from 120s to 5min. Orchestrator heartbeat bumps `updatedAt` unconditionally for live games (not just when signals exist) to prevent stale-data false positives.

...
- **Consolidated Analytics Pipeline**: All signal persistence routes through `trackPlay()` in `server/services/playTracker.ts` → `persisted_plays` table. MLB signals auto-persist from the engine cycle in `liveGameOrchestrator.ts` (no user request needed) with in-memory dedup guard (`mlbPersistGuard`). NBA signals persist via `trackPlay()` in the live-signals and halftime endpoints. The old `play_alerts` table is read-only (legacy settling via `autoResolveAlerts` still runs for historical data, but no new writes). Manual save endpoint (`POST /api/plays`) also routes through `trackPlay()`. Grading runs via `gradePersistedPlays` every 3 minutes.

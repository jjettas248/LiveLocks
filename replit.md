...
- **MLB Prop Engine** (`server/mlb/`): Gold Master live prop engine for MLB player prop markets. Implements seven market calculators (Hits, Total Bases, Batter K, Pitcher K, Hits Allowed, HR, HRR) via a modular pipeline: types → feature engineering → projection → probability → calibration → market calculators. Key features:
    ...
    - **MLB Signal Integrity**: Unified signal qualification — game card `signalCount`, detail view, edge feed, and dashboard signal counts ALL use the same qualification logic: `canShowSignal` + edge >= 5% + not suppressed. Game card count capped at 3 (matching `MAX_SIGNALS_PER_GAME`). Edge feed falls back to `mlbEdgeCache` when `mlbSignalsCache` is empty (no detail view visit required). `calibratedProbabilityOver/Under` are 0-100 scale (not 0-1). Empty states use honest messaging explaining thresholds. Calculator disabled when pitchers unresolved.

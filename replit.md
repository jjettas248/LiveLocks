...
- **HR Radar Settlement Repair (May 2026)**: Six fixes wired into the HR Radar settlement loop to stop signaled HRs from disappearing/falsely-missing/false-uncalled. (1) `liveGameOrchestrator.gradeSingleHRPlay` now calls `liveSignalBus.cashSignal` against BOTH `mlb:<gameId>:<playerId>:hrr:OVER` (canonical) and `…:home_runs:OVER` (legacy back-compat) — previously hardcoded `home_runs` always missed because canonical mapper emits `hrr`. (2) `shared/canonicalSignal.LIFECYCLE_TRANSITIONS` now allows `watch→cashed` and `build→cashed` (previously rejected, silently dropping low-tier cashes). (3) `storage.ensureHrRadarAlertHit` race fallback rescues qualified alerts (`confidenceTier ∈ {strong,elite}` OR `signalState ∈ {actionable,fire}` OR `alertTier ∈ {officialAlert,prepare}`) as `called_hit (userVisible=true)` instead of hidden `late_signal`. (4) `storage.reconcileHrRadarAlertsForGame` skips alerts with `hitInning != null` and uses inning-strict-less-than comparison when `hrData.half === "final"` (synthetic). (5) `matchDecision` presence-only `called_miss` rows tagged with `[presence-only]` marker; `resolveHrRadarAlertAsHit` sets `userVisible: !isPresenceOnly`. (6) `gradeSingleHRPlay` synchronously appends synthetic `stage_attack`/`stage_building` qualifying signal_event row anchored 1s before `hrEnd` when engine was past PATH A-E at HR moment, closing the same-tick race. CRITICAL ESM FIX: top-of-file `import * as` declarations replace previously-broken `require()` calls in `liveGameOrchestrator.ts` (5 sites) — every prior `require("./hrRadarOutcomeStamp")` etc. was throwing `ReferenceError: require is not defined` under ESM and being silently swallowed by surrounding try/catch, which is why `bus.cashSignal` NEVER fired in production despite being in the code path.
...
- **MLB HR Radar READY → FIRE Promotion**: `server/mlb/hrRadarUserStage.ts` exports `maybePromoteReadyToFire(stage, ctx)` and `STRONG_HR_DRIVER_SIGNALS`. Wired into `enrichWithUserStage` AFTER `strongerStage(legacyMapped, suggested)`. Upgrades userStage="ready"→"fire" when ANY of: (A) displayScore10≥9.5 + canonicalStage="attack" + ≥1 strong driver (`elite_barrel|two_hard_hit_balls|massive_single_contact|pitcher_collapse_power`), (B) `alertPath ∈ PATH_PROMOTES_TO_FIRE` (FAST_PROMOTE_ELITE) + signalState ∈ {live, actionable}, (C) dynamicState="BET_NOW" + displayScore10≥9.5 + not stale (current ≥ 0.85×peak readiness). Never demotes; never runs on stages other than ready; conviction-cap-aware (PATH_F_BLOCKED_BRIDGE @ 6.0/10 cannot promote via Rule A/C). Diagnostics: `[HR_RADAR_READY_TO_FIRE]` (JSON: rule, drivers, score, paths) and `[HR_RADAR_FIRE_BLOCKED]` (JSON: missing pieces — gated to score≥9.5 to bound noise). Test: `npx tsx server/mlb/hrRadarReadyToFire.test.ts` (21 invariants).
- **MLB HR Watch Peak Detection (May 2026)**: `server/mlb/nearHrContact.ts` adds `detectNearHrContactPeak(events, windowSize=5)` that scans the last N at-bats (most-recent first) and returns the STRONGEST near-HR tier seen (`lean > watch > null`) with a `sourceAbIndex`. `server/mlb/liveGameOrchestrator.ts:1902` swaps the prior single-AB `_lastAbForNearHr.slice(-1)[0]` call for the windowed scan. Real-world miss it fixes: Brandon Valenzuela TOR @ HOU (5/9) AB#3 was a textbook lean (104.4 EV / 24° LA / 382 ft / barrel / xBA .830) but AB#4 was a flat 99.6 mph 6° liner — old "last AB only" path saw AB#4, returned `tier=null`, the alert peakReadinessScore stayed at 0, the +6 score bump never fired, the 35→25 HR_WATCH_GATE drop never fired, and the eventual HR was graded `uncalled_hr`. New peak path remembers AB#3's lean across AB#4, so the alert promotes correctly. Pure-additive: `detectNearHrContact` thresholds, the +3/+6 bump, the 35→25 gate drop, and probability math all unchanged. Diagnostics: `[MLB_HR_WATCH_DETECTED]` payload now includes `sourceAbIndex`, `scannedWindow`, `totalAbs` so audits can see WHICH AB triggered the lean/watch.

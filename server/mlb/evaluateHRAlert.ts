...
export function evaluateHRAlert(input: HRAlertInput): HRAlertResult {
  ...
  // Edge decoupled (2026-06) — the tier/level/signalState are NEVER changed by
  // sportsbook edge. (Resolved merge with origin/main's data-quality-aware EV
  // demote: HR Radar is occurrence-only, so the demote branch is intentionally
  // removed; main's `diagnostics.dataQuality` from the core evaluator is kept.)
  return result;
}

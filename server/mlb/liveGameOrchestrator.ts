...
function gradeSingleHRPlay(
  ...
  // classify WHY it was/wasn't caught into an explainable review bucket. Pure
  ...
  try {
    ...
    const existingOutcomeStatus =
      hrRadarOutcomeStampMod.getHrRadarOutcomeStamp(gameId, playerId)?.outcomeStatus ?? null;
    ...
    // Best-effort context flags from the engine's accumulated drivers (robust to
    // scale; no-op false when absent). Never blocks classification.
    ...
    // The HR's own swing: was it genuinely barrel / HR-shaped? Read from the HR's
    // own AB entry in the live contact cache (the resolver drops it from preHrAbs,
    // but here we inspect it directly). A barrel/HR-shaped HR with no prior live
    // evidence is `same_pa`; a cheap/wall-scraper HR with no signal falls through
    // to true_uncalled/context. When the HR AB isn't in the cache yet, default to
    // true (a HR is hard contact) — dataQuality gating handles the missing case.
    ...
    const review = classifyHrReview({
      ...
      // Map peak→official timestamps: a peak in an official dynamic state before
      // the HR is a surfaced-official-signal proxy (consistent with the existing
      // reachedHrMaxWindowPeak grading in this same function).
...
export class LiveGameOrchestrator {
  ...
  private buildHRAlertInput(p: {
    ...
    return {
      ...
      pullRatePercent: p.playerContact?.pullRatePercent ?? null,

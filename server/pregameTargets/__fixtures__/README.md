...
| Plate **win attribution** (given a known `hitHr`) | **[COVERED]** | `plateWinAttribution.json` (`deriveWinAttribution`: hit/miss, public/not, first-AB, third-AB, unknown-first-AB). This is the *attribution* step **after** `hitHr` is decided — not the grader that decides it |
| Plate **grading** (`resolveOutcome`: `hitHr`/`totalBases` from box score, pre-`final` miss guard, TB exactness) | **[NEEDS-EXTERNAL]** | `shadowOutcomes.resolveOutcome` + `gradePregameOutcomes` read a live box-score line map + draft/`gameStatus` state; not reproducible credential-free (see Live grading feeds below) |
...
| Mound **outcome derivation** — push / DNP / missing / stat-correction **flip** | **[COVERED]** | `mlbMoundGrading.json` — pure `deriveMoundOutcome` / `deriveMoundMarketOutcome` / `deriveModelOutcomeLabel`; `correction_before`→`correction_after` proves a corrected `actual` flips missed→cashed on identical inputs. The **stateful regrade** of an already-settled signal is NOT this row (see below) |
| Mound **stateful regrade** after an official stat correction | **[NEEDS-EXTERNAL]** | `gradeMoundOutcomes` skips a `status === "graded"` signal unless it is a live-win refresh or direction repair (`moundShadowOutcomes.ts`), so re-grading a settled signal on a corrected final needs a persisted signal + storage → Postgres integration test (PR2), not a pure fixture |
| Stale timestamps / odds-age freshness | **[COVERED]** | Derivation locked directly in `nbaFreshOdds.json` (`deriveFreshOdds`: the strict `< 600s` cutoff — 599 fresh, 600 stale — and fail-closed non-finite/unknown → stale). Locked end-to-end in `nbaComputeProbability.json` (`odds_fresh_599`/`odds_stale_600`/`odds_unknown_stale`: `oddsAgeSec` observably changes `ceilingReason`/`calibrationTrack` to `stale_or_unknown_odds`), with a self-check asserting the 599/600 outputs diverge. Finalizer's downstream stale cap covered in `nbaFinalize.json` → `stale_odds_gate`. Wall-clock stamps normalized to `<volatile>` |
...
| Null behavior | **[COVERED]** | `missing_assist_rates`, `filters_invalid` (null line/prob/edge), Mound grading null-final / null-line / null-direction. `canonicalize` encodes `NaN`/`±Infinity` as distinct sentinels (not `null`) so a null→non-finite regression in these null-locked fixtures can't slip through `JSON.stringify`; a self-check asserts the distinctness |
| Deterministic ordering | **[COVERED]** | `nbaEngineWrapper.json` → `ordering_multi` (unsorted candidates). For `buildResponse`/`buildMoundResponse` the golden inputs are fed **ascending** by `score10` (suppressed before public) so the descending sort must reorder them — deleting the production `sort((a,b)=>b.score10-a.score10)` flips the `include_suppressed`/`include_research` output and fails byte-equality; a self-check also asserts descending output from ascending input |
...
| Live grading feeds | **[NEEDS-EXTERNAL]** | `gradePregameOutcomes` / `gradeMoundOutcomes` read live box-score/play-by-play + storage and mutate persisted signal state — including Plate `resolveOutcome` (deciding `hitHr`/`totalBases`, the pre-`final` miss guard, TB exactness) and the Mound already-graded **skip-unless-refresh/repair** regrade path. Only the pure outcome-derivation math either side of these is frozen (`mlbMoundGrading.json`, `plateWinAttribution.json`). |
